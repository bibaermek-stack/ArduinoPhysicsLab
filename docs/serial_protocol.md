...
## 9. TODO: checksum (болашақ кезең)

V1.0 хаттамасында checksum жоқ (әдейі жеңілдетілген). Болашақта
берілісті тексеру қажет болса:

- Пакетке қосымша кілт түрінде қосылады, мыс. `EXP=E02,U=5.0,...,CHK=A3`.
- `PacketParser`-ге бөлек тексеру қадамы қосылады (checksum сәйкес
  келмесе — `errors`-ке жазба, `is_valid=False`).
- Бұл өзгеріс артқа үйлесімді (backward-compatible) болуы тиіс: checksum
  жоқ ескі пакеттер де әлі де қабылдана алатындай, не жаңа хаттама
  нұсқасы (`PROTO=2`) арқылы ажыратылатындай жобаланады.
